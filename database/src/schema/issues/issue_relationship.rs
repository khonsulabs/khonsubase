...
pub struct IssueRelationship {
    ...
    pub issue_started_at: Option<DateTime<Utc>>,
    ...
    pub async fn list_for<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        let rows = sqlx::query!(
            r#"SELECT 
                ...
                CASE WHEN issue_a.id = $1 THEN issue_b.started_at ELSE issue_a.started_at END as issue_started_at,
        ...
        Ok(rows
            ...
            .map(|row| Self {
                ...
                issue_started_at: row.issue_started_at,
